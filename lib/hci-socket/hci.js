...
Hci.prototype.init = function() {
  ...
  if (process.env.HCI_CHANNEL_USER) {
    ...
    // only reset on armhf snap
    if (process.env.SNAP_ARCH == 'armhf') {
      this.reset();
    }
...
Hci.prototype.pollIsDevUp = function() {
  ...
  setTimeout(this.pollIsDevUp.bind(this), 5000);
};
...
Hci.prototype.onSocketData = function(data) {
  try {
    ...
    console.log("onSocketData error:" + err);
    console.log("data=" + data.toString('hex'))
    this.emit('stateChange', 'socketError')
  }
};
...
Hci.prototype.processLeAdvertisingReport = function(count, data) {
  for (var i = 0; i < count; i++) {
    ...
    var eirLength = data.readUInt8(8); //8 bytes
